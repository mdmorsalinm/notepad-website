...
onAuthStateChanged(auth, async (user) => {
    // notesContainer.innerHTML = ''; // Clear the notes container to avoid duplication

    if (user) {
        showLoggedInView(); // Show logged-in view

        // Fetch posts for the logged-in user
        const querySnapshot = await getDocs(query(collection(db, "posts"), where("uid", "==", user.uid)));
        console.log("HELLOW");

         const postsArray = querySnapshot.docs.map((doc) => doc.data().body);

            // Now, iterate over this array
            postsArray.forEach((postBody) => {
                createPostBoxes(postBody); // Add the posts to the UI
                console.log("TESTING!");
            });
        

        // Handle saving posts when user clicks the save button
        saveBtn.addEventListener("click", () => {
        ...
        // User is not logged in, show the logged-out view
        showLoggedOutView();
